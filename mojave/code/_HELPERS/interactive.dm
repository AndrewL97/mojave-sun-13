...
/proc/do_after_interactive(mob/user, time = 3 SECONDS, atom/target, timed_action_flags = NONE, progress = TRUE, datum/callback/extra_checks, interaction_key, max_interact_count = 1, bonus_time = 0, focus_sound = null, type = /obj/effect/client_image_holder/progress_focus)
	if(!user)
		return FALSE
...
/obj/effect/client_image_holder/progress_focus
	name = "focus"
	...
	var/obj/effect/client_image_holder/extra_effect
...
/obj/effect/client_image_holder/progress_focus/Initialize(mapload, mob/target_mob, datum/progressbar/target_bar, bonus_time, f_sound)
	. = ..()
...
/obj/effect/client_image_holder/progress_focus/proc/build_extra_effect()
	extra_effect = new /obj/effect/client_image_holder/progress_trap(get_turf(src), target, linked_bar, time_per_click)

/obj/effect/client_image_holder/progress_focus/attackby(obj/item/I, mob/user, params)
	. = ..()
	on_boost(user)
	uses += 1
	if (uses >= max_uses)
...
/obj/effect/client_image_holder/progress_focus/proc/on_boost(mob/user)
...
/obj/effect/client_image_holder/progress_focus/Destroy()
	. = ..()
	QDEL_NULL(extra_effect)

/obj/effect/client_image_holder/progress_trap
	name = ""
	desc = ""
	image_icon = 'mojave/icons/effects/interactive.dmi'
...
/obj/effect/client_image_holder/progress_trap/Initialize(mapload, mob/target_mob, datum/progressbar/target_bar, time_delta)
	. = ..()
	target = target_mob
	linked_bar = target_bar
	loss_per_click = -1 * time_delta

/obj/effect/client_image_holder/progress_trap/attackby(obj/item/I, mob/user, params)
	. = ..()
	linked_bar.boost_progress(loss_per_click)
	user.playsound_local(user, 'sound/machines/buzz-sigh.ogg', 50, TRUE)

/obj/effect/client_image_holder/progress_focus/skillcheck
	name = "skill check"
...
/obj/effect/client_image_holder/progress_focus/skillcheck/Initialize(mapload, mob/target_mob, datum/progressbar/target_bar, bonus_time, f_sound)
	. = ..()
	extra_effect.SpinAnimation(spin_speed, -1)
	start_time = REALTIMEOFDAY

/obj/effect/client_image_holder/progress_focus/skillcheck/build_extra_effect()
	extra_effect = new /obj/effect/client_image_holder/skill_marker(get_turf(src), target)

/obj/effect/client_image_holder/progress_focus/skillcheck/on_boost(mob/user)
	var/current_time = REALTIMEOFDAY

	var/time_diff = (current_time - start_time) % spin_speed
	var/degrees = time_diff * 360/spin_speed
	var/obj/effect/client_image_holder/skill_marker/temp_effect = new(get_turf(src), target)
...
/obj/effect/client_image_holder/skill_marker
	name = ""
	desc = ""
	image_icon = 'mojave/icons/effects/interactive.dmi'
